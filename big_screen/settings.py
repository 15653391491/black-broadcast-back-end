...
LOGBAK_DIR = os.path.dirname(os.path.dirname(os.path.dirname(BASE_DIR))) + '/logBak/'
...
CACHES = {
    ...
    "test": {  # 首页图表数据
        "BACKEND": "django_redis.cache.RedisCache",
        "LOCATION": "redis://127.0.0.1:6380/4",
        "OPTIONS": {
            "CLIENT_CLASS": "django_redis.client.DefaultClient",
        }
    }
...
CELERY_IMPORTS = ('con_brocast.tasks', 'bigScreen.task', 'con_control.tasks', "con_setting.tasks")
...
CELERYBEAT_SCHEDULE = {  # 定时器策略
    # 定时任务一：　每隔一小时运行一次
    u'首页图表': {
        "task": "bigScreen.task.set_bigscreen_chart",
        "schedule": schedules.timedelta(hours=2),
        "args": (),
    },
    u'数据中心图表': {
        "task": "con_control.tasks.save_chart_data",
        "schedule": schedules.timedelta(hours=24),
        "args": (),
    },
    u'数据中心热力图': {
        "task": "con_control.tasks.heat_map_data",
        "schedule": schedules.timedelta(hours=24),
        "args": (),
    },
    u'清理过期黑广播数据': {
        "task": "con_brocast.tasks.expire_broadcast",
        "schedule": schedules.timedelta(minutes=1),
        "args": (),
    },
    u'清理过期心跳包': {
        "task": "con_setting.tasks.pop_heartbeat",
        "schedule": schedules.timedelta(minutes=1),
        "args": (),
    },
}
